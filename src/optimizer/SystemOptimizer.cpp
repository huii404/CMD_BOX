...
// --- HỆ THỐNG DỌN RÁC THEO NHIỆM VỤ (TASK-BASED DISK CLEAN) ---

// 1. Nhiệm vụ: Dọn rác tạm bề mặt & Cache người dùng (Temp, CrashDumps, WER User, INetCache, RecycleBin, Flush DNS)
long long SystemOptimizer::cleanSurfaceAndUserTemp() {
    ...
    // Cache người dùng mới: CrashDumps của User & App thứ ba
    threads.emplace_back([this]() { sc.runCMD("cmd /c del /f /s /q \"%LocalAppData%\\CrashDumps\\*\" 2>nul"); });

    // Cache người dùng mới: WER cá nhân (Temp, ReportArchive, ReportQueue)
    ...
    // Cache người dùng mới: INetCache (tệp tạm webview & ứng dụng ngầm người dùng)
...
// 2. Nhiệm vụ: Dọn rác Trình duyệt & Ứng dụng (Chrome, Edge, Brave, Discord, Shader)
long long SystemOptimizer::cleanBrowserAndAppCache() {
    long long before = 0, after = 0;
    try { before = fs::space("C:\\").available; } catch (...) {}
...
// 3. Nhiệm vụ: Dọn dẹp Chuyên sâu Hệ thống & Tồn dư Cập nhật (DISM WinSxS, Windows.old, $WINDOWS.~BT, Logs, LiveKernelReports)
long long SystemOptimizer::cleanDeepSystemAndUpdates() {
    long long before = 0, after = 0;
    try { before = fs::space("C:\\").available; } catch (...) {}
    ...
    // Tồn dư cập nhật bản lớn ($WINDOWS.~BT, $WINDOWS.~WS, Windows.old)
    ...
    // Nhật ký nâng cấp Windows & LiveKernelReports
    batContent += "del /f /s /q \"%SystemRoot%\\Panther\\*.*\" 2>nul\n";
    batContent += "del /f /s /q \"%SystemRoot%\\LiveKernelReports\\*.*\" 2>nul\n";

    // Dọn dẹp chuẩn hệ thống DISM WinSxS & Windows Update tệp tải dở
...
// 4. Nhiệm vụ: Dọn rác Môi trường lập trình (node_modules, pip, gradle, VS Code...)
long long SystemOptimizer::cleanDevArtifactsAndCaches() {
...
void SystemOptimizer::multiTierDiskClean() {
    while (true) {
        sc.cls();
        cout << "\n\n"
             << " [1] Dọn rác bề mặt & Cache người dùng (Temp, CrashDumps, WER, DNS, Thùng rác)\n"
             << " [2] Dọn rác Trình duyệt & Ứng dụng (Chrome, Edge, Firefox, Discord...)\n"
             << " [3] Dọn dẹp Chuyên sâu & Tồn dư Cập nhật (Windows.old, $WINDOWS.~BT, DISM, Logs)\n"
             << " [4] Dọn rác Môi trường lập trình (node_modules, Pip, Gradle, VS Code...)\n"
             << " [5] [⚡] Dọn toàn diện Hệ thống (1 + 2 + 3)\n"
             << " [6] [🚀] Dọn tất cả (Cả 4)\n"
             << " [0] Quay lại\n\n"
             << " [Chọn]: ";
 ...
 */
// Hàm xóa tận gốc thư mục trên Windows: Xử lý Read-Only, System, Hidden và đường dẫn dài MAX_PATH (>260 ký tự)
static bool forceDeleteFolder(const fs::path &folderPath) {
    if (!fs::exists(folderPath)) return true;

    string pathStr = folderPath.string();
    std::replace(pathStr.begin(), pathStr.end(), '/', '\\');

    string extendedPath = pathStr;
    if (extendedPath.rfind("\\\\?\\", 0) != 0) {
        if (extendedPath.length() >= 2 && extendedPath[1] == ':') {
            extendedPath = "\\\\?\\" + extendedPath;
        }
    }

    // 1. Đệ quy gỡ bỏ thuộc tính Read-Only, System, Hidden cho toàn bộ file/folder con
    std::error_code ec;
    try {
        for (auto it = fs::recursive_directory_iterator(folderPath, fs::directory_options::skip_permission_denied, ec);
            ...
            try {
                ...
                string subExt = p.string();
                std::replace(subExt.begin(), subExt.end(), '/', '\\');
                if (subExt.rfind("\\\\?\\", 0) != 0 && subExt.length() >= 2 && subExt[1] == ':') {
                    subExt = "\\\\?\\" + subExt;
                }
                SetFileAttributesA(subExt.c_str(), FILE_ATTRIBUTE_NORMAL);
            } catch (...) {}
            it.increment(ec);
        }
    } catch (...) {}

    SetFileAttributesA(extendedPath.c_str(), FILE_ATTRIBUTE_NORMAL);

    // 2. Thử xóa bằng std::filesystem::remove_all
    fs::remove_all(folderPath, ec);
    if (!fs::exists(folderPath)) return true;

    // 3. Nếu chưa sạch (do symlinks hoặc kẹt tiến trình), dùng lệnh rd /s /q an toàn
    string cmd = "cmd.exe /d /c \"rd /s /q \"" + pathStr + "\"\" >nul 2>&1";
    system(cmd.c_str());
    if (!fs::exists(folderPath)) return true;

    // 4. Nếu vẫn còn, ép buộc xóa bằng PowerShell Force
    string psCmd = "powershell -NoProfile -NonInteractive -Command \"Remove-Item -LiteralPath '" + pathStr + "' -Recurse -Force -ErrorAction SilentlyContinue\" >nul 2>&1";
    system(psCmd.c_str());
...
void SystemOptimizer::cleanDevCaches(bool interactive) {
    ...
    // Tự động tìm kiếm các thư mục dự án / workspace trên TẤT CẢ các ổ đĩa (C:, D:, E:, G:...)
    std::vector<fs::path> scanRoots;
    
    // Thư mục chạy hiện tại
    ...
    // Lấy danh sách các ổ đĩa logic có sẵn (Fixed drives)
    DWORD driveMask = GetLogicalDrives();
    for (char c = 'C'; c <= 'Z'; ++c) {
        if (driveMask & (1 << (c - 'A'))) {
            string rootDrive = string(1, c) + ":\\";
            UINT driveType = GetDriveTypeA(rootDrive.c_str());
            if (driveType == DRIVE_FIXED) {
                // Quét các thư mục code/dự án phổ biến trên từng ổ đĩa
    ...
    // Thêm các thư mục code phổ biến trong UserProfile
    if (!baseUser.empty()) {
    ...
    if (hasPython) {
        ...
        int pyDeleted = 0;
        for (const auto &sr : scanRoots) {
            pyDeleted += cleanDirectoryArtifacts(sr, 
    ...
    int nodeDeleted = 0;
    for (const auto &sr : scanRoots) {
        nodeDeleted += cleanDirectoryArtifacts(sr, 
