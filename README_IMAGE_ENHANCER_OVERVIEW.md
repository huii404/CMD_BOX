...
| **Phân rã đa tần số** | **2-Scale Guided Filter**<br>• Micro ($r=1, \epsilon=300$)<br>• Macro ($r=3, \epsilon=1200$)<br>• Trọng số tĩnh: $1.35 / 0.65$ | **3-Scale Guided Filter**<br>• Nano ($r=0.5, \epsilon=80$)<br>• Micro ($r=1$)<br>• Macro ($r=3$)<br>• Trọng số thích ứng động theo `localFreq` | ⭐⭐⭐⭐⭐ Tách biệt được chi tiết siêu mịn (lông tơ, vân da, gân lá mảnh) mà không làm thô khối lớn. |
| **Khử nhiễu & Giới hạn Coring** | **Cauchy Continuous Coring** tĩnh:<br>$k = 12.0$ cố định toàn ảnh | **Noise-Floor Adaptive Coring**:<br>Hệ số $k \in [6.0, 40.0]$ thích ứng theo MAD (Median Absolute Deviation) $7\times7$ | ⭐⭐⭐⭐⭐ Chống biến nhiễu hạt (ISO cao, JPEG artifact) thành "sạn giả chi tiết". |
